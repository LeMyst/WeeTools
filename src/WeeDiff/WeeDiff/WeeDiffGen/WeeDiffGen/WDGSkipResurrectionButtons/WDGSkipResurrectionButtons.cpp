...
#include "WDGSkipResurrectionButtons.h"
...
LPWDGPLUGININFO WDGPlugin::GetPluginInfo()
{
	...
	{
		TEXT("Skip Resurrection Buttons"),
		TEXT("Skips the ressurection button whenever you die or use Token of Siegfried."),
		TEXT("[UI]"),
		TEXT(""),
		TEXT("Shinryo"),
		1,
		0,
		{ 0xb7bfc158, 0x2d86, 0x4678, { 0xbc, 0x89, 0xea, 0xf5, 0x1c, 0x90, 0xa3, 0x4a } }
	};
...
DiffData *WDGPlugin::GeneratePatch()
{
	...
	try
	{
		...
		sFindData.lpData = "68 C5 1D 00 00";
	...
	{
		sprintf_s(szMsg, 256, "WDGSkipRessurectionButtons :: Part 1 :: %s", lpszMsg);
		m_dgc->LogMsg(szMsg);
		return NULL;
	}
	...
	{
		sprintf_s(szMsg, 256, "WDGSkipRessurectionButtons :: Part 2 :: %s", lpszMsg);
		m_dgc->LogMsg(szMsg);
		return NULL;
	}
