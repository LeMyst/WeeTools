...
INT32 WeePluginManager::LoadPlugin(LPCTSTR lpszPluginPath, WeeGUI *gui, USHORT unMajorVersion, USHORT unMinorVersion)
{
	...
	WeePlugin::fnInitPlugin fnInitPlugin = (WeePlugin::fnInitPlugin)GetProcAddress(hPlugin, "InitPlugin");
	if(fnInitPlugin == 0)
	{
		FreeLibrary(hPlugin);
	...
	IBasePlugin *wdp = (IBasePlugin *)fnInitPlugin((WeePlugin::IGUI *)gui, unMajorVersion, unMinorVersion);
	if(wdp == 0)
	{
		FreeLibrary(hPlugin);
	...
	WeePlugin::LPWEEPLUGININFO lp_wpi = wdp->GetPluginInfo();
	if(lp_wpi == 0)
	{
		FreeLibrary(hPlugin);
		return LOADPLUGIN_FAILED;
...
INT32 WeePluginManager::UnloadPlugin(WeePlugin::HPLUGIN hPlugin)
{
	...
	if(it != m_plugins.end())
	{
		WeePlugin::IPlugin *wdp = it->second.lpInterface;
