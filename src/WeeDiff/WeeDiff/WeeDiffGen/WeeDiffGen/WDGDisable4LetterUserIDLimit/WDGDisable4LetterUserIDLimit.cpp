...
#include "WDGDisable4LetterUserIDLimit.h"
...
LPWDGPLUGININFO WDGPlugin::GetPluginInfo()
{
	...
	{
		TEXT("Disable 4 Letter User ID Limit"),
		TEXT("Removes the limit of min. 4 characters for the account name."),
		TEXT("[Fix]"),
		TEXT(""),
		TEXT("Shinryo"),
		1,
		0,
		{ 0x52397995, 0x3980, 0x421d, { 0x96, 0x4d, 0x5f, 0x7d, 0x46, 0x4d, 0xc5, 0x89 } }
	};
...
DiffData *WDGPlugin::GeneratePatch()
{
	...
	try
	{
		...
		uOffset = offsets[1];
		ZeroMemory(&sFindData, sizeof(sFindData));
		sFindData.lpData = "83 AB 04";
		sFindData.chWildCard = '\xAB';
		sFindData.uStart = uOffset + 13;
		sFindData.uMask = WFD_PATTERN | WFD_WILDCARD;
	...
	{
		sprintf_s(szMsg, 256, "WDGDisable4LetterUserIDLimit :: Part %d :: %s", uPart, lpszMsg);
		m_dgc->LogMsg(szMsg);
		return NULL;
	}
