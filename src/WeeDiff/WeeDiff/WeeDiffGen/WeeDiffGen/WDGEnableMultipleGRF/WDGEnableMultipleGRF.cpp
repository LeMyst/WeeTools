...
#include "WDGEnableMultipleGRF.h"
...
DiffData *WDGPlugin::GeneratePatch()
{
	...
	try
	{
		...
		{
			memcpy(sFindData.lpData, "\x68\x00\x00\x00\x00\xB9\xAB\xAB\xAB\x00\x88\xAB\xAB\xAB\xAB\x00\xE8\xAB\xAB\xAB\xAB\x8B\xAB\xAB\xAB\xAB\x00", 27);
			memcpy(sFindData.lpData + 1, (CHAR *)&uGRF, 4);
		...
		UINT32 uAddPak = m_dgc->Raw2Rva(uOffset + 10 + uSkip) + m_dgc->GetDWORD32(uOffset + 11 + uSkip) + 5;
		...
		{
			ZeroMemory(&sFindData, sizeof(sFindData));
			sFindData.lpData = new CHAR[10];
			sFindData.uDataSize = 10;

			memcpy(sFindData.lpData, "\xE8\x00\x00\x00\x00\x90\x90\x90\x90\x90", 10);
			...
			memcpy(sFindData.lpData, "\x90\x90\x90\x90\x90", 5);

			m_dgc->Replace(CBAddDiffData, uOffset + 10 + uSkip, &sFindData);
		}
		else
		{
			ZeroMemory(&sFindData, sizeof(sFindData));
